...
reduce paths(.allOf?) as $path (.;
  (
    getpath($path) | .
    + ( [.allOf[], .] | map({ minItems: (.minItems // empty) }) | max )
    + ( [.allOf[], .] | map({ maxItems: (.maxItems // empty) }) | min )

    + ( [.allOf[], .] | map({ minLength: (.minLength // empty) }) | max )
    + ( [.allOf[], .] | map({ maxLength: (.maxLength // empty) }) | min )

    + ( [.allOf[], .] | map({ minProperties: (.minProperties // empty) }) | max )
    + ( [.allOf[], .] | map({ maxProperties: (.maxProperties // empty) }) | min )
...
reduce paths(.["x-kubernetes-int-or-string"]? == true) as $path (.;
