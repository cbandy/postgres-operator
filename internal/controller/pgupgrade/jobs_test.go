    ...
    spec:
      ...
      - command:
        ...
        - |-
          ...
          old_bin=$(for dir in '/usr/lib/postgresql/19/bin' '/usr/libexec/postgresql19' '/usr/pgsql-19/bin'; do
            [ -d "${dir}" ] && PATH="${dir}${PATH+:${PATH}}"
          done && command -v postgres)
          old_bin="${old_bin%/postgres}"
          new_bin=$(for dir in '/usr/lib/postgresql/25/bin' '/usr/libexec/postgresql25' '/usr/pgsql-25/bin'; do
            [ -d "${dir}" ] && PATH="${dir}${PATH+:${PATH}}"
          done && command -v initdb)
    ...
    spec:
      ...
      - command:
        ...
        - |-
          ...
          delete() (set -x && rm -rf -- "$@")
          old_data="${data_volume}/pg${old_version}"
          control=$(for dir in '/usr/lib/postgresql/19/bin' '/usr/libexec/postgresql19' '/usr/pgsql-19/bin'; do
            [ -d "${dir}" ] && PATH="${dir}${PATH+:${PATH}}"
          done && LC_ALL=C pg_controldata "${old_data}")
