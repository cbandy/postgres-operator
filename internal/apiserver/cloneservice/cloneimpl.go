...
func Clone(request *msgs.CloneRequest, namespace, pgouser string) msgs.CloneResponse {
	...
	_, err = apiserver.Clientset.CrunchydataV1().Pgclusters(namespace).Get(request.TargetClusterName, metav1.GetOptions{})
	if err == nil {
		response.Status.Code = msgs.Error
	...
	_, err = apiserver.Clientset.CrunchydataV1().Pgtasks(namespace).Create(task)
