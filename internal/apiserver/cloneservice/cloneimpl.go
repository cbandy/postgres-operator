...
func Clone(request *msgs.CloneRequest, namespace, pgouser string) msgs.CloneResponse {
	...
	_, err = apiserver.PGOClientset.CrunchydataV1().Pgclusters(namespace).Get(request.TargetClusterName, metav1.GetOptions{})
	if err == nil {
		response.Status.Code = msgs.Error
	...
	_, err = apiserver.PGOClientset.CrunchydataV1().Pgtasks(namespace).Create(task)
